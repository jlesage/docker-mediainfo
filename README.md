...
This project provides a Docker container for [MediaInfo](https://mediaarea.net/en/MediaInfo).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of MediaInfo.
...
[Releases]: https://github.com/jlesage/docker-mediainfo/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/mediainfo/tags
...
container, it would proxy all HTTP requests for `mediainfo.domain.tld` to
...
server {
	...
	location /mediainfo/ {
		...
		# Needed when audio support is enabled.
		location /mediainfo/websockify-audio {
			proxy_pass http://docker-mediainfo/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of MediaInfo integrated into Docker image. |
...
Any desktop notification generated by MediaInfo is forwarded to the
